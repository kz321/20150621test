...
var request = $.ajax({
	url: "https://163.15.192.205/vany/index.php/mobile/readMsg/format/json",
	type: "POST",	
	cache: false,
	crossDomain: true,
	success: 
		function (data, status) {
			...
			for($i = 0; $i< res.length;$i++){
				if(res[$i]['level'] == '緊急'){
					$("#emergency").append('<li data-role="list-divider">' + res[$i]['timestamp'] + '</li>'
				...
				else if(res[$i]['level'] == '警告'){
					$("#warning").append('<li data-role="list-divider">' + res[$i]['timestamp'] + '</li>'
						+'<li data-icon="false"><a href="#"  data-ajax="false"><label id="sender">寄件者 : ' 
						+ res[$i]['sender'] + '</label><label id="message">'+ res[$i]['message'] + '</label>' 
						+ '<label id="info">' + res[$i]['info'] + '</label></a></li>').listview('refresh');				
				}
				else if(res[$i]['level'] == '通知'){
					$("#notification").append('<li data-role="list-divider">' + res[$i]['timestamp'] + '</li>'
